...
IMService.prototype.connect = function (currentUser) {
    ...
    let userData = {
        ...
        avatar: this.currentUser.attrs && this.currentUser.attrs.avatar ? this.currentUser.attrs.avatar : '../../../static/images/goodsDelivery/shipUserLogo.png',
		userId: this.currentUser.userId
    }
	// console.log('goEasy账户信息', currentUser);
	// console.log('账户code', this.currentUser.attrs && this.currentUser.attrs.userCode ? this.currentUser.attrs.userCode : this.currentUser.userCode);
    //初始化相关的监听器
    this.initialListeners();
    this.goEasy.connect({
        id: this.currentUser.attrs && this.currentUser.attrs.userCode ? this.currentUser.attrs.userCode : this.currentUser.userCode,
        data: userData,
        onSuccess: function () {
            //连接成功
...
IMService.prototype.initialListeners = function () {
	let code = this.currentUser.attrs && this.currentUser.attrs.userCode ? this.currentUser.attrs.userCode : this.currentUser.userCode;
    //监听私聊消息
    this.goEasy.im.on(this.GoEasy.IM_EVENT.PRIVATE_MESSAGE_RECEIVED, (message) => {
		// console.log('监听received', message);
        //更新私聊消息记录
        let friendId;
        if (code === message.senderId) {
