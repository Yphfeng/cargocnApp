...
<script>
	...
	export default {
		...
		onLoad(options) {
			...
			this.checkedGroup = Number(options.type);
			...
			this.getPartnerData();
			if(this.checkedGroup == 1){
				this.checkedUser.userName = getApp().globalData.pransportGoods.shippingUser.userName ? getApp().globalData.pransportGoods.shippingUser.userName : '';
				this.checkedUser.userId = getApp().globalData.pransportGoods.shippingUser.userId ? getApp().globalData.pransportGoods.shippingUser.userId : '';
			} else {
				this.checkedUser.userName = getApp().globalData.pransportGoods.carrierUser.userName ? getApp().globalData.pransportGoods.carrierUser.userName : '';
				this.checkedUser.userId = getApp().globalData.pransportGoods.carrierUser.userId ? getApp().globalData.pransportGoods.carrierUser.userId : '';
			}
		},
		onReady() {
		},
		...
		methods: {
			...
			toSave() {
				...
				if(this.checkedGroup == 1){
					getApp().globalData.pransportGoods.shippingUser = this.checkedUser;
				} else {
					getApp().globalData.pransportGoods.carrierUser = this.checkedUser;
				}
