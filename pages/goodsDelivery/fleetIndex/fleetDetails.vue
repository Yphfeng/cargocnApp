<template>
	<view class="newFleetDetails">
			...
			<view class="car_source_item" v-if="item.serviceType == 10">
				...
				<view class="listBtnBox" v-if="trajectoryPermissions || collectionPermissions || callDriverPermissions">
					...
					<view class="userListBtnGroup">
						...
						<view class="btnBox" v-if="callDriverPermissions">
							<image class="icon" src="/static/images/newFleetDetails/mobile.png"></image>
							<view class="msg" @click="toCallUser(item, index)">联系TA</view>
						</view>
					</view>
				</view>
			</view>
		...
		<uni-popup ref="feetMemberHintPopup" type="center">
			<view class="nfd-fleetAddMsgPopupBox">
				<view class="nfd-fleetAddMsgTitle"><text class="nfd-fleetAddMsgTitleText">{{memberHintMsg}}</text></view>
				<view class="nfd-fleetAddMsgBtn" @click="closeFleetMemberHintPopup"><text class="nfd-fleetAddMsgBtnText">我知道了</text></view>
			</view>
		</uni-popup>
	...
	export default {
		data() {
			return {
				...
				selectedStartProvinceIndex: 0,
				selectedStartCityIndex: -1,
				startCityArr: [],
				popupEndCityParams: {},
				selectEndCityParams: {},
				selectedEndProvinceIndex: 0,
		...
		methods:{
			...
			toTrackSimple(item) {
				...
				uni.navigateTo({
					url: './map?index=0&dateTime=' + dateTime + '&endTime=' + endTime + '&selectedMapItem=' + JSON.stringify(item)
				})
			},
			showName(item) {
				var that = this;
			...
			toResetSelectData() { // 重置选择的省市区
				...
				this.selectedStartCityIndex = -1;
				this.popupStartCityParams = {};
				this.selectedEndProvinceIndex = 0;
				this.selectedEndCityIndex = -1;
				this.popupEndCityParams = {};
				this.pushDiyCityItem(this.provinceArr[0].name, this.provinceArr[0].code, 0);
			},
			...
			makePhoneCall(result, item, index) {
				...
				if(result == 1) {
					uni.showModal({
						title:'提示',
						content: '申请获取您的电话权限，用于联系司机使司机快速加入车队',
						success: function(res) {
							if (res.confirm) {
								uni.makePhoneCall({
