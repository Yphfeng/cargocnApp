				...
				<view class="commonlyList">
					<view class="addressBox">
						...
						<view class="addressDistanceBox">
							...
							<view class="distanceMsg" v-if="!item.estimatedDistance && (Number(getDistance(item.child).toFixed(0)) == 0 || Number(getDistance(item.child).toFixed(0)) < 0)"></view>
		...
		methods: {
			...
			bindGoodsChange(e) {
				...
				if (goodsVal[0] !== proIndex) {
					uni.$emit("changeHisCityAry", {ary: provinceArr[proIndex].cities});
			...
			find() {
				...
				uni.$emit("historySearch", searchInfo);
			},
			deleteParamsMsg(flag) {
				...
				uni.showLoading();
				uni.$emit("historySearch", newParams);
			},
			deleteCreatedDt() {
				let newParams = this.params;
				newParams.createdDt = '';
