<template>
  <view class="container">
    ...
    <view class="text" v-if="status === '-500'">人脸识别认证中</view>
    <view class="text" style="color: #94eb66;font-size: 36rpx" v-if="status === 1">人脸识别认证成功</view>
    <view class="text" style="color: red;font-size: 36rpx" v-if="status === 0">{{failMsg}}</view>
                          ...
                          if (faceRes.retCode == "0000000") {
							...
							if (that.type == "updateMobile") {
								...
								uni.navigateBack({
								  delta: 1,
								  success: function() {
									  prevPage.$vm.mobileUp();
								  }
								});
								return;
							}
                            that.status = 1
							prevPage.$vm.pageStep = 1;
							prevPage.$vm.getIdentifyingDetails();
							...
							var type = this.type;
							uni.navigateTo({
								url: './faceResult?type=success&msg=认证成功&source=' + type
							})
							// uni.showToast({
							// 	title: '认证成功',
							// 	icon: 'success',
							...
							that.failMsg = faceRes.retDesc;
                            that.status = 0
                            prevPage.$vm.getFaceResult(0, faceRes.retDesc);
							var type = this.type;
							uni.navigateTo({
								url: './faceResult?type=error&msg=' + faceRes.retDesc + '&source=' + type
							})
                            // setTimeout(() => {
                            //   uni.navigateBack({
                            //     delta: 1
