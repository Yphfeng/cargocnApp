<template>
	<view class="carDetailNewMainPage backgroundView">
	  <view class="error" v-if="showErrMsg()">{{showErrMsg()}}</view>
	  ...
	  <view class="infoView">
		<view class='nameView' @click="toCarView">
		  ...
		  <text class="textTitle">车型车长</text>
		  <view class="arrow_right">
			  <view class="text">{{commonCarModel}}</view>
			  <view class="text">{{commonCarLength}}</view>
			  <image src="../../../static/images/newRightIcon.png" class="rightIcon"></image>
			</view>
		</view>
	    <view class='nameView'>
	      <span style='color:red;'>*</span>
	      <text class="textTitle">车牌号</text>
	...
	<view class="infoView">
		...
		<view class='nameView'>
		  ...
		  <picker class="pickerDate" mode="selector" :range="vehicleTypeArr" range-key="name" :value="vehicleTypeIndex"  @change="bindVehicleType">
			...
			<view v-else class="defaultDate">请选择车辆类型</view>
		  </picker>
		</view>
		<view class='nameView'>
		  <span style='color:red;'>*</span>
		  <text class="textTitle">外廓尺寸</text>
		  <input placeholder="外廓尺寸" placeholder-style="font-size:28rpx;" v-model="params.gabarite"></input>
		</view>
		<view class='nameView'>
		  <span style='color:red;'>*</span>
		  <text class="textTitle">车牌颜色</text>
		  <picker class="pickerDate" mode="selector" :range="vehicleColorArr" range-key="name" :value="vehicleColorIndex"  @change="bindVehicleColor">
		  	<view v-if="params.vehiclePlateColor">{{ params.vehiclePlateColor }}</view>
		  			<view v-else class="defaultDate">请选择车牌颜色</view>
		  </picker>
		</view>
		<view class='nameView'>
		  <span style='color:red;'>*</span>
		  <text class="textTitle">车辆能源类型</text>
		...
		<view class='nameView newTimePicker'>
			<span style='color:red;'>*</span>
			<text class="textTitle">行驶证注册日期</text>
		...
		<view class='nameView newTimePicker'>
			<span style='color:red;'>*</span>
			<text class="textTitle">行驶证发证日期</text>
		...
		<view class='nameView newTimePicker'>
			<text class="textTitle">行驶证有效期至</text>
		...
		<view class='nameView'>
		  <text class="textTitle">行驶证档案编号</text>
	...
	<view class="infoView" v-if="isCollapse">
		...
		<view class='nameView newTimePicker'>
			...
			<!-- <picker class="pickerDate" mode="date" :value="params.operationExpire" :start="startDate" :end="endDate" @change="bindDateChange3">
				...
				<view v-else class="defaultDate">许可证有效期至</view>
			</picker> -->
			<uni-datetime-picker
			  type="date"
				:border="false"
		...
		<view class='nameView'>
		  <text class="textTitle">挂车牌照号</text>
		  <input placeholder="挂车牌照号" maxlength="20" placeholder-style="font-size:28rpx;" v-model="params.gPlateNo"></input>
		</view>
		<view class='nameView'>
		  <text class="textTitle">是否挂靠</text>
...
<script>
	...
	export default {
		...
		onLoad(options) {
			...
			//获取车辆详情信息
			var plateNo = JSON.parse(carInfo).plateNo
			this.oldPlateNo = plateNo;
			...
			async getVehicleDetail(plateNo) {
				try{
					...
					if (res.retCode == "0000000") {
						...
						if (res.rspBody.vehicleCapacity) {
							this.params.vehicleCapacity = res.rspBody.vehicleCapacity
						} else {
							this.params.vehicleCapacity = res.rspBody.checkWeight
						}
						if (res.rspBody.licenseMainImg) {
							this.faceImage = res.rspBody.licenseMainImg
						}
						if (res.rspBody.licenseSubImg) {
							this.backImage = res.rspBody.licenseSubImg
						}
						
						if (res.rspBody.licenseBackImg) {
							this.sideImage = res.rspBody.licenseBackImg
						}
						if (res.rspBody.vehicleImg) {
							this.vehicleImg1 = res.rspBody.vehicleImg;
						}
						if (res.rspBody.transPermitMainImg) {
							this.transPermitMainImg1 = res.rspBody.transPermitMainImg
						}
						if (res.rspBody.transPermitSubImg) {
							this.transPermitSubImg1 = res.rspBody.transPermitSubImg
						}
						if (res.rspBody.gLicenseMainImg) {
							this.gVehicleMainImg1 = res.rspBody.gLicenseMainImg
						}
						if (res.rspBody.gLicenseSubImg) {
							this.gVehicleSubImg1 = res.rspBody.gLicenseSubImg
						}
						if (res.rspBody.isHang == 1) {
							this.index = 1
						} else {
							this.index = 0
						}
		   ...
		   uploadFrontImage: function(path) {
			...
			uploadFile(path, flag) {
				...
				.then(result => {
					...
					if (result.retCode == '0000000') {
						...
						if (flag == 3) {
							//道路运输证主页
							that.params.transPermitMainImg = result.rspBody[0].url;
							that.transPermitMainImg1 = result.rspBody[0].url;
						} else if (flag == 4) {
							// 车辆照片
							that.vehicleImg1 = result.rspBody[0].url;
							that.params.vehicleImg = result.rspBody[0].url;
						} else if (flag == 5) {
							//行驶证反面
							that.sideImage = result.rspBody[0].url;
			...
			saveInfoSelect() {
				...
				if (isCollapse) {
					...
					// 		return;
					// 	}
					// 	if (!params.transPermitValid) {
					// 		uni.showToast({
					// 			title: '请选择道路运输许可证有效期至',
					// 			icon: 'none'
					// 		})
					// 		return;
					// 	}
				...
				if (parm.plateNo != this.oldPlateNo) {
					parm.oldPlateNo = this.oldPlateNo;
				}
				//司机的driverId
