					...
					<swiper-item>
						<view class="swiper-content">
							...
							<view class="search-content">
									...
									<input type="text" v-model="arriveInputVal" @input="arriveInput"  placeholder="请输入省/市/直辖市" class="search-input" />
								
									<image src="/static/images/ship/car-search-clear.png" mode="widthFix" class="car-search-clear" @click="clearEnd"></image>
								
							</view>
							<view class="search-history-common" ></view>
	...
	export default {
		...
		created() {
			...
			if (!provinceLocal) {
				...
				console.log(this.provinceLocal[0], '啊啊啊啊')
		...
		methods: {
			...
			toArrive(item) {
				...
				console.log(startHistory.length,'length')
				...
				console.log(startHistory,'startHistorystartHistory')
			...
			toSearchResult(item) {
				...
				console.log(arriveHistory,'arriveHistory')
...
<style lang="scss" scoped>
	.alert-content {
		...
		.scroller-content {
			height: calc(75vh - 214rpx);
			padding-left: 36rpx;
			box-sizing: border-box;
			&.end {
				height: calc(75vh - 254rpx);
			}
		}
		.search-result-list {
			height: calc(75vh - 214rpx);
