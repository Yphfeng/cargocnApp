	...
	<view class="source-setting">
		...
		<view class="search-source-item" :style="{height: contentHeight + 'rpx'}">
			<view class="search-source-item-main">
				<view class="search-source-title">我的车辆</view>
				<view class="search-add" @click="showNewCarAlert">
					<image src="/static/images/ship/source-add-1.png" class="source-add"></image>
					新增车辆</view>
			</view>
			<view :style="{height: (contentHeight - 110) + 'rpx'}" v-if="hasSearchedVehicleArr.length > 0">
				<scroll-view class="search-content-center-scroller" :scroll-y="true" :style="{height: (contentHeight - 110) + 'rpx'}">
					<view class="search-content-router-scroll">
						<view class="search-content-item" v-for="(item, index) in hasSearchedVehicleArr">
							<view class="search-item-inner" :class="{active: isSelectedVehicleIndex == index}" @click="switchVehicle(index)">
								...
								<!-- <text>{{item.carLen.typeName}}米{{item.carType.typeName}}</text> -->
		...
		<view class="search-source-item" :style="{height: contentHeight + 'rpx'}">
			<view class="search-source-item-main">
				<view class="search-source-title">我的线路</view>
				<view class="search-add" @click="addRoute">
					<image src="/static/images/ship/source-add-1.png" class="source-add"></image>
					新增线路
				</view>
			</view>
			<view :style="{height: (contentHeight - 110) + 'rpx'}" v-if="hasSearchedRouterArr.length > 0">
				<scroll-view class="search-content-router-center" :style="{height: (contentHeight - 110) + 'rpx'}"  :scroll-y="true">
					<view class="search-content-router-scroll">
			...
			confirmDel() {
				...
				.then(res => {
					...
					if (res.retCode == "0000000") {
						...
						that.getVehicleSettingsInfo(); //刷新首页的展示
					} else {
						uni.showToast({
							title: res.retDesc,
							icon: 'none'
...
<style lang="scss" scoped>
	.source-setting {
		...
		.search-source-item {
			...
			.search-content-center-scroller {
				display: flex;
				flex-direction: row;
				flex-wrap: wrap;
				box-sizing: border-box;
