	...
	<view class="content">
		<view class="input-whole">
			...
			<picker class="picker" mode="multiSelector" :value="dateTime" @change="changeDateTime"  @columnchange="changeDateTimeColumn" :range="dateTimeArray">
				<view class='lableBox'>
					<view class="choose-value">{{params.senderDt}}</view>
				</view> 
			</picker>
		</view>
	...
	export default {
		...
		methods: {
			...
			initTime(){
				...
				this.params.senderDt= this.createTimeStr(this.dateTimeArray,this.dateTime);
			},
			...
			async loadPan() {
				...
				params.loadDt = params.senderDt;
				uni.showLoading();
				try{
					var res = await ship.postRequest("/yd/task/loadingFinish", params)
					uni.hideLoading();
