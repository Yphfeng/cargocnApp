	...
	<view style="width: 100vw;height: 100vh;" class="shipIndex" :class="[{noScoller: isNoScroll}]">
		...
		<view v-if="showGuide == 2">
			...
			<!-- <view class="China_Travel" @mousemove="move2" @touchmove.stop="move2" :style="{ top: position.y2 + 'px', left: position.x2 + 'px' }" :class="hideFixedIcon ? 'hide_icon':''" v-if="ChinaTravelObj.show && avticeIconObj.ChinaTravelFlag" >
				<image src="https://hyzg-app.oss-cn-beijing.aliyuncs.com/ChinaTravel.png" @click="showLocationPower" class="advers-close"></image>
			...
			<view class="index" :class="[{noScoller: isAlert || isFleetAlert || isAlertShow || remindStatus}]">
				...
				<view class="tabWhole">
					...
					<view class="indexHead">
						<view class="positioning" @click="showGetLocation">
						...
						<view class="side">
							...
							<image class="scan-box" mode="widthFix"  src="/static/images/ship/scan.png" @click="requestPower"></image>
				...
				<view class="tabContent_car" :style="getTabContent_carStyle()" v-show="carSourceIndex == 0">
					<carSource ref="carSourceRef"></carSource>
					<!-- <businessCard ref="businessCardRef"></businessCard> -->
...
<script>
	...
	export default {
		...
		async onLoad(options) {
			var that = this;
			console.log('avticeIconObj',this.avticeIconObj)
			...
			uni.getSystemInfo({
			    success: function (res) {
					...
					console.log(screenHeight3,screenWidth3)
			    }
			});
		...
		onPullDownRefresh() {
			...
			console.log(carSourceIndex, '搜索index111111')
			if (carSourceIndex == 0) {
				this.$refs.carSourceRef.refresh();
				// this.$refs.businessCardRef.refresh();
				return;
		...
		onReachBottom() {
			var that = this;
			var carSourceIndex = this.carSourceIndex;
			console.log('滚动到底部加载更多', carSourceIndex)
			if (carSourceIndex == 0) {
				this.$refs.carSourceRef.loadMore();
				return;
			}
			var isTabIndex = this.isTabIndex;
			...
			if (isTabIndex == 0) {
				...
				var xie1 = this.xie1;
				if (!dataParams02.vehicleLength && !dataParams02.vehicleModel && zhuang1.length < 1 && xie1.length < 1) {
					//无搜索条件则执行
					if (platformQuoteList.length == 20) {
				...
				var dataParams04 = this.dataParams04;
				var zhuang2 = this.zhuang2;
				var xie2 = this.xie2;
				if (!dataParams04.vehicleLength && !dataParams04.vehicleModel && zhuang2.length < 1 && xie2.length < 1) {
					//无搜索条件则执行
					if (platformQuoteList.length == 20) {
	...
	methods: {
	  ...
	  showLocationPower() {
		  ...
		  uni.showModal({
		  	title: '提示',
			content: '为了收集您点亮的位置，需访问您的位置权限',
			success: function(res) {
				if(res.confirm) {
					that.goBtnChinaTravel();
				}
			}
		  })
	  },
      ...
      async goBtnChinaTravel(){
		...
		console.log("loactionFlag", loactionFlag);
        if(!loactionFlag){
          return
        }
        console.log(info)
        ...
        // })
			...
			async getRecondMore(type, tabIndex) {
				...
				if (tabIndex == 0) {
					...
					//获取我的所有车队和加入的车队
					var fleet = await service.newQueryPostRequest("/cd/fleet/list", {pageSize:100,pageNum:1,isMyFleet:1, verificationStatus: 1});
					...
					// var fleetArr = fleet.rspBody.items;
					// var relateFleetArr = relateFleet.rspBody.items;
					// var fleetAll = fleetArr.concat(relateFleetArr);
					var fleetAll = fleet.rspBody.items;
					
					dataParams02.fleetSids = fleetAll.length > 0 ? JSON.stringify(fleetAll) : '';
				...
				var platformQuote = await service.newQueryPostRequest("/td/circuit/getDriverPtmOrdersAround", params);
					...
					if (platformQuote.retCode == "0000000") {
						...
						if (tabIndex == 0) {
							var platList = this.platformQuoteList;
							var arr = [];
							platformQuoteList.forEach(atem => {
								var findIndex = platList.findIndex(item => {
									return item.sid == atem.sid
								})
							...
							if (type == 'refresh') {
								...
								this.recondArr = arr;
								var total = this.total;
								this.isNoData02 = (total < 1 && arr.length < 1) ? true : false;
								this.isNoMore02 = (platformQuoteList.length < 10 && total == platList.length) ? 'noMore' : 'more'
							}  else {
								this.recondArr = this.recondArr.concat(arr);
								this.isNoMore02 = platformQuoteList.length < 10 ? 'noMore' : 'more'
							}
							...
							var arr = [];
							platformQuoteList.forEach(atem => {
								var findIndex = fleetQuoteList.findIndex(item => {
									return item.sid == atem.sid
								})
								if (findIndex < -1) {
									arr.push(atem)
							...
							if (type == "refresh") {
								...
								this.isNoData04 = (total1 < 1 && arr.length < 1) ? true : false;
								this.isNoMore04 = (platformQuoteList.length < 10 && total1 == fleetQuoteList.length) ? 'noMore' : 'more'
							} else {
								this.recondArr1 = this.recondArr1.concat(arr);
								this.isNoMore04 = platformQuoteList.length < 10 ? 'noMore' : 'more'
			...
			showArriveAddress(item) { // 目的地
			   let obj = {
				province: item.provinceName4 ? item.provinceName4 : item.provinceName3,
				provinceCode: item.provinceCode4 ? item.provinceCode4 : item.provinceCode3,
				city: item.cityName4 ? item.cityName4 : item.cityName3,
				cityCode: item.cityCode4 ? item.cityCode4 : item.cityCode3,
				area: item.areaName4 ? item.areaName4 : item.areaName3,
				areaCode: item.areaCode4 ? item.areaCode4 : item.areaCode3,
			   }
			   return this.showAddressMsg(obj);
			},
			//获取平台货源的数据
						...
						if (tabIndex == 0) {
							...
							if (type == 'refresh') {
								...
								if (platformQuoteList.length < 10) {
									this.getRecondList("refresh", tabIndex);
								} else {
									this.isNoMore02 = "more"
								}
							}
							...
							if (type == "refresh") {
								this.isNoData04 = platformQuoteList.length < 1 ? true : false
								this.isNoMore04 = 'more'
								...
								if (platformQuoteList.length < 10) {
									this.getRecondList("more", tabIndex);
								} else {
									this.isNoMore04 = "more"
								}
							}
							//格式化平台列表数据
							this.formatPlatformQuoteList(platformQuoteList, 'fleet', type);
						}
			...
			showVehicleLength(item) {
				...
				orderVehicleLength = orderVehicleLength == '-1.0' || orderVehicleLength == -1 ? '不限车长' : orderVehicleLength + '米';
			...
			showGetLocation() {
				...
				if (plus.navigator.checkPermission('android.permission.ACCESS_FINE_LOCATION') === 'undetermined') {
					uni.showModal({
						title: '提示',
						content: "为了按距离您的位置远近向您展示平台的货源，需获取您的位置信息",
						success: function(res) {
							if(res.confirm) {  
								that.getLocation();
							} else {  
			...
			goScan() {
				...
				if(platform == 'ios'){
					...
					uni.showModal({
						title: '提示',
						content: '扫码需获取您的相机权限和存储权限，用于通过货源码来查询货源信息',
						success: function(res) {
							if (res.confirm) {
								wxScanModule.scanActivity({}, (data) => {
									console.log('扫描结果', data)
			...
			getTabContent_carStyle() {
				...
				// var mrt = statusBarHeight*2;
				// return "height:" + height + 'rpx;'
				// return "margin-top:" + mrt + 'rpx'
				var height = (windowHeight - statusBarHeight)* 2 - 102;
				var mrt = statusBarHeight*2 + 102;
			...
			// 		uni.navigateTo({
			// 			url:"/pages/redPacket/index/index?pageName=carSource"
			// 		})
			// 	}
