	...
	<view class="search">
		<view class="searchHeader">
			...
			<view class="searchInputBox">
				...
				<input :focus="onFocus" type="text" :value="searchVal" placeholder="搜索" class="searchInput" @input="inputName" @confirm="toSearch()" />
				<view v-if="searchVal.length>0" @click="cleanValue" class="seachClear">
					<image style="width: 25rpx; height: 25rpx;" src="../../static/images/newCancel.png" mode=""></image>
				</view>
				<view class="searchBtn">
					<view class="addr" :class="{active: searchCondition == 1}" @click="toSearchType(1)">地址</view>
					<view class="dotted" />
					<view class="addr"  :class="{active: searchCondition == 0}" @click="toSearchType(0)">
						名称
					</view>	
			...
			<view class="searchDetails" v-for="(item, index) in searchData" :key="index" :class="index == 0 ? 'firstDetails' : 'otherDetails'" @click="jumpToMap(item.address, item.title)">
				<view class="leftMsg">
					<view :class="index == 0 ? 'firstMsgIcon' : 'otherMsgIcon'">
						<image v-if="item.searchCondition == 1" src="../../static/images/searchIcon/positioning.png"></image>
						<image v-else src="../../static/images/searchIcon/search.png"></image>
					</view>
					<view class="msgBox">
						<view class="title">{{item.title}}</view>
						<view v-if="item.searchCondition == 1" class="address">{{item.address ? item.address.formatted_address : ""}}</view>
					</view>
				</view>
...
<script>
	...
	export default {
		...
		onLoad(option) {
			...
			this.searchCondition = searchCondition ? searchCondition : 0;
			...
			switch(this.searchType) {
				case 'service':
					if (this.searchCondition == 1) {
						this.storageKeyName = 'serviceHistoryDataFromAddr';
					} else {
						this.storageKeyName = 'serviceHistoryDataFromName';
					}
		...
		methods: {
			...
			toSearch() { // 搜索成功后先将搜索内容存入缓存中后再进行地图跳转
				...
				if(!this.searchVal && this.searchCondition == 1) {
					uni.showToast({
					    title: '请输入要搜索地址！',
					    duration: 2000,
						icon: 'none'
				...
				let that = this;
				//根据地址搜索
				console.log(this.searchCondition, 'aaaa');
				if (this.searchCondition == 1) {
					...
					if (this.searchVal) {
						uni.setStorage({
							...
							success: function (res) {
								...
								uni.$emit("searchFromName", {providerName: that.searchVal});
							}
						});
					} else {
						uni.switchTab({
			...
			jumpToMap(item, title) { // 跳转地图
				...
				if (this.searchCondition == 1) {
					...
					switch(this.searchType) {
						case 'service':
							getApp().globalData.serviceSearch.searchCondition = 1;
							...
							uni.navigateTo({
								url:'/pages/index/mapView/mapView1'
							})
							break;
						case 'roadCondition':
					...
					switch(this.searchType) {
						case 'service':
							getApp().globalData.serviceSearch.searchCondition = 0;
							getApp().globalData.serviceSearch.searchVal = title ? title : this.searchVal;
							getApp().globalData.serviceSearch.latitude = "";
							getApp().globalData.serviceSearch.longitude = ""; //按服务站搜索清空定位数据
							...
							if (this.searchVal) {
								...
								uni.$emit("searchFromName", {providerName: title ? title : ''});
							}
