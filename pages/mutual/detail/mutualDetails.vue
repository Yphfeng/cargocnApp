	...
	<view class="newRoadDetails" :style="{overflow: commitInputShow ? 'hidden' : 'auto', 'padding-top': (statusBarHeight + 35) + 'px'}">
		<view class="newHeader">
			...
			<view class="headBox">
				...
				互助详情
				<view class="headRight" v-if="detailsObj.userId == userId && detailsObj.examStatus == 0" @click="editMutual">
		...
		<view class="serviceStationBox">
			<view class="mainCommentsBox">
				...
				<view class="solveBox" :style="showStatusBc(detailsObj.solveFlag)">
					{{showStatus(detailsObj.solveFlag)}}
				</view>
		...
		<view class="commentsBox">
			...
			<view class="commentsTransition">
				...
				<view class="commentsTitle" style="margin-top:30rpx;">所有评价（{{detailsObj.commentCount}}）</view>
		...
		<view class="commentsFoot" :style="safeAreaHeight > 0 ? 'padding-bottom:60rpx;' : 'padding-bottom:22rpx;'">
			<!-- <view class="footBtn" @tap="likeChange(1)">
				<image class="footImg" :src="detailsObj.isLiked == 1 ? '../../../static/images/already.png' : '../../../static/images/praise.png'"></image>
				{{detailsObj.likeNum}}
			</view>
			<view class="line"></view>
			<view class="footBtn" @tap="likeChange(2)">
				<image class="footImg" :src="detailsObj.isDisLiked == 1 ? '../../../static/images/badReview.png' : '../../../static/images/stepOn.png'"></image>
...
<script>
	...
	export default {
		...
		onShow() {
			...
			// let that = this;
			// uni.onKeyboardHeightChange(res => {
			// 	that.commentBoxHeight = res.height;
			// 	if (res.height == 0) {
			// 		that.commitInputShow = false
		...
		methods: {
			getDetail(){ // 获取主体详情
				var that = this;
				utils.postRequest('/mu/help/detail', {
					id: that.serviceId,
				})
				.then(res=>{
					console.log('详情', res);
			...
			getMyCommentList() { // 我的评论
				...
				utils.postRequest('/mu/helpcomment/list', {
					helpId: that.serviceId,
					isMyComment: 1,
					pageSize: 1,
					pageNum: 1,
				})
			...
			showSubComments(item, key) { // 查看更多2级回复
				...
				utils.postRequest('/mu/help/comment/reply/list', params)
				.then(res=>{
			...
			submit(val) { // 发表评论
				...
				utils.postRequest('/mu/help/comment/reply/save', params)
				.then(res=>{
					console.log('回复', res);
					if(res.retCode=="0000000"){
						uni.showToast({
			...
			commentslike(item, flag, type) { // 列表--点赞/取消点赞
				...
				utils.postRequest('/mu/help/like/save', params)
				.then(res=>{
					if(res.retCode=="0000000"){
						// uni.showToast({
						// 	title: '操作成功',
						...
						item.upvoteNum = item.isLiked == 1 ? item.upvoteNum + 1 : item.upvoteNum - 1;
						if(type == 'my') {
							for(let i in that.commentList) {
								if(item.id == that.commentList[i].id) {
									that.commentList[i].isLiked = item.isLiked;
									that.commentList[i].upvoteNum = item.isLiked == 1 ? that.commentList[i].upvoteNum + 1 : that.commentList[i].upvoteNum - 1;
								}
							}
						}
						if(type == 'first') {
							if(item.id == that.myCommentList.id) {
								...
								that.myCommentList.upvoteNum = item.isLiked ? that.myCommentList.upvoteNum + 1 : that.myCommentList.upvoteNum - 1;
							}
						}
			...
			jumpToReply() { // 普通回复需跳转回复页面
				...
				uni.navigateTo({
					url: '../../index/serviceDetails/detailsReply?type=mutual&id=' + this.serviceId
				})
			},
			...
			showStatusBc(status) {
				switch(status) {
					case 0:
						return 'border:1px solid gba(119, 119, 119, 0.2);color:#777777;';
					case 1:
						return 'border:1px solid rgba(0, 119, 255, 0.52);color:#0077FF;';
					case 2:
						return 'border:1px solid rgba(224, 32, 32, 0.39);color:#E02020;';
				}
			},
			backToFromPage() {
				uni.navigateBack({
			...
			editMutual() {
				//编辑
				uni.navigateTo({
					url: '../publish/index?pageName=互助编辑&detailsObj=' + JSON.stringify(this.detailsObj)
				})
			}
		}
	}
